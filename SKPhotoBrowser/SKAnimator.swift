...
class SKAnimator: NSObject, SKPhotoBrowserAnimatorDelegate {
    ...
    func willDismiss(_ browser: SKPhotoBrowser) {
        guard let sender = browser.delegate?.viewForPhoto?(browser, index: browser.currentPageIndex) ?? senderViewForAnimation,
    ...
    func presentAnimation(_ browser: SKPhotoBrowser, completion: ((Void) -> Void)? = nil) {
        ...
        UIView.animate(
            ...
            completion: { (Bool) -> Void in
                UIApplication.shared.setStatusBarHidden(!SKPhotoBrowserOptions.displayStatusbar, with: .fade)
